...
    if (action === "link") {
        if (cmd.args.length < 3) {
            cmd.replyToAuthor('usage: `xbl link "Your Gamer Tag"`');
            return;
        }
        Duder.startTyping(cmd.channelID);

        // --- START: FIX ---
        // Slice the args array from the 2nd element to the end, then join to capture the full gamertag.
        var gamertag = cmd.args.slice(2).join(" ");
        // --- END: FIX ---
...
        if (json.profileUsers === undefined || json.profileUsers.length === 0) {
...
            if (setting.id === "Gamertag" && setting.value.toLowerCase() === gamertag.toLowerCase()) {
